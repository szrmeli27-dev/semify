...
export const useMusicPlayer = create<MusicPlayerState>()(
  persist(
    (set, get) => ({
      ...
      toggleLike: async (track) => {
        ...
        if (alreadyLiked) {
          ...
          if (userId) {
            await supabase.from('liked_songs').delete().eq('user_id', userId).eq('track_id', track.id)
          }
        } else {
      ...
      createPlaylist: async (name) => {
        ...
        if (data) {
          set((s) => ({
            playlists: [...s.playlists, { id: data.id, name: data.name, tracks: [] }],
          }))
        }
      },
    ...
    }),
    {
      ...
      partialize: (state) => ({
        volume: state.volume,
        // Don't persist user data - it's loaded from DB
      }),
    }
  )
)
